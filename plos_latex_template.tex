...
%The procedure outlined in Equations~(\ref{eqn:onehotmale},~\ref{eqn:onehotfemale}) is %known  one-hot encoding 
...
\begin{equation}
\label{eq:hhazard}
\lambda(\mathbf{X}, t_{i}) = P(Y = t_{i}|Y \geq t_{i}, \mathbf{X}),
\end{equation}
the probability that, if someone has survived up until month $t_{i}$, they will die in that month.
...
\begin{equation}
\label{eq:pmf}
P(Y = t_{j} | \mathbf{X}) = \lambda(\mathbf{X}, t_{j}) \prod_{i=1}^{j-1} (1 - \lambda(\mathbf{X}, t_{i}))
\end{equation}
Knowing $P(Y = t_{j} | \mathbf{X}) $ for all $t_{j}$ gives the 
...
{i=1}^{k} P(Y = t_{i} | \mathbf{X}) $ is the cumulative density function correponding to the probability mass function in equation~\ref{eq:pmf}~\cite{downey}.
